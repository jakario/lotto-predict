...
def download_image(url, filename):
    try:
        ...
        print(f"⚠️ Error {os.path.basename(filename)}: {e}")
        return False
...
# Exact confirmed URLs from the sheet's source
cards = {
...
print("Final Tarot Image Download (Direct Source)...")
for filename, url in cards.items():
    filepath = os.path.join(tarot_dir, filename)
    download_image(url, filepath)
    time.sleep(1) # Be gentle with Wikimedia

print("\n--- FINAL CHECK ---")
all_files = [f for f in os.listdir(tarot_dir) if f.endsWith('.jpg')]
